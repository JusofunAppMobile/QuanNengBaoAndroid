 ...
 * <p/>
 * <p/>
 * The {@link android.widget.ScrollView} class also takes care of its own scrolling, so does not
...
public class OverScrollView extends FrameLayout implements OnTouchListener {
    ...
    private VelocityTracker veloctiy=VelocityTracker.obtain();
    @Override
    public boolean onTouchEvent(MotionEvent ev) {

        veloctiy.computeCurrentVelocity(2000);
        ...
        switch (action & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (mIsBeingDragged) {
                    ...
                    float nowy=ev.getY();
                    mLastMotionY = y;
                    indexy = (int) (nowy - lastY)/3;
                    Log.e("index","y"+y+"\n"+"lastY"+lastY);
                    Log.e("indexy",indexy+"");
                    if (indexy>=0 ) {
                        if (indexy<255) {
                            if (onScrollListener != null) {
                                onScrollListener.onPullScroll((int) (indexy));
                            }
                        }
                    } else {
                        if (isOverScrolled()) {
                            ...
                            scrollBy(0, deltaY / 2);
                            Log.e("isOverScrolled", deltaY / 2 + "");
                        } else {
                            scrollBy(0, deltaY);
                            Log.e("OverScrolled", deltaY + "");
                        }
                    }
                }
                break;
    ...
    public void animation(int y){
        ...
        anim.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                indexy=y;
                if (onScrollListener != null)
                    onScrollListener.onPullScroll(indexy);
                Log.e("indexanim",y+"");
            }
        });
        anim.start();
    ...
    protected void onScrollChanged(int leftOfVisibleView, int topOfVisibleView, int oldLeftOfVisibleView, int oldTopOfVisibleView) {
        Log.e("tag", "onScrollChanged-leftOfVisibleView=" + leftOfVisibleView);
        Log.e("tag", "onScrollChanged-topOfVisibleView=" + topOfVisibleView);
        Log.e("tag", "onScrollChanged-oldLeftOfVisibleView=" + oldLeftOfVisibleView);
        Log.e("tag", "onScrollChanged-oldTopOfVisibleView=" + oldTopOfVisibleView);

        if (onScrollListener != null) {
            onScrollListener.onScrollChangedListener(leftOfVisibleView, topOfVisibleView, oldLeftOfVisibleView, oldTopOfVisibleView);
        }
